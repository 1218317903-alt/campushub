...
public interface DocumentTaskMapper {
     ...
     *
     * <h2>哪些任务可以被重置</h2>
     * 两类，都是"人工重试确实能改变什么"的情形：
     * <ol>
     *   <li><b>终态</b>（{@code SUCCEEDED} / {@code FAILED}）：已经跑完了，
     *       重跑是唯一有意义的动作。</li>
     *   <li><b>{@code PENDING} 且 {@code attempt_count > 0}</b>：这是一次失败之后
     *       <b>正在等退避窗口</b>的任务。它与"刚入队、从未尝试过"的任务
     *       （{@code attempt_count = 0}）在数据上有明确区别，而人工点击
     *       "重新解析"的语义恰恰是"别等了，现在就跑"。
     *       漏掉这一支的后果不是报错，而是响应 200、状态 {@code PENDING}、
     *       界面在最长两分钟内毫无变化 —— 调用方无法区分"已经受理"与
     *       "什么都没发生"，而用户会去点第二次、第三次。</li>
     * </ol>
     *
     * <p>唯一被排除的是 {@code RUNNING}：重置一个正在执行的任务，
     * 会让它在结束后再跑一遍（同一份字节解析两遍），
     * 并让第一次的结果在租约条件上落空。
     *
     * @param documentId 文档自增主键
     * @param taskType   任务类型
