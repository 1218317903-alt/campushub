...
help:
	...
	@echo ""
	@echo "  端到端"
	@echo "    make e2e           真实浏览器验收（需 up + run + fe-dev 都在跑）"
	@echo ""
...
## 端到端
## ---------------------------------------------------------------------------
## 真实浏览器验收（Phase 05 起）。用本机 Chrome 驱动，不下载额外浏览器。
## 前提：make up && make run && make fe-dev 三者都在跑。
## 本机注意：后端需显式指定端口（见 docs/11 §7.6），否则前端代理会 502。
.PHONY: e2e
e2e:
